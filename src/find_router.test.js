...
describe('Find router v1', function() {
  ...
  beforeAll(function () {
    ...
    r.get('/adm/api/exp', local_middlewares, function (ctx, reply) {
      reply.send(ctx.get('req').method+' /adm/api/exp');
    });

    r.get('/adm/api/exp/:poll_id', local_middlewares, function (ctx, reply) {
      reply.send(ctx.get('req').method+' /adm/api/exp/1');
    });

    r.post('/adm/api/exp', local_middlewares, function (ctx, reply) {
      reply.send(ctx.get('req').method+' /adm/api/exp');
    });

    r.put('/adm/api/exp/:poll_id', local_middlewares, function (ctx, reply) {
      reply.send(ctx.get('req').method+' /adm/api/exp/'+ctx.get('params').poll_id);
    });

    r.delete('/adm/api/exp/:poll_id', local_middlewares, function (ctx, reply) {
      reply.send(ctx.get('req').method+' /adm/api/exp/'+ctx.get('params').poll_id);
    });
  ...
  test('middlewares', async function () {
    ...
    expect(counter_call).toHaveBeenCalledTimes(23);
